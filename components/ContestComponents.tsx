...
import { Users, Calendar, Trophy, ChevronRight, DollarSign, Flame } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="relative h-56 w-full overflow-hidden">
        <img src={contest.image} alt={contest.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
        ...
        <div className="absolute bottom-4 left-4 right-4">
           <h3 className="text-xl font-bold text-white mb-1 line-clamp-1 group-hover:text-primary-300 transition-colors">{contest.title}</h3>
        </div>
      </div>
      
      <div className="p-6 flex-grow flex flex-col">
        <p className="text-gray-600 dark:text-gray-400 text-sm mb-6 line-clamp-2 flex-grow leading-relaxed">{contest.description}</p>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
           ...
           <div className="flex items-center text-sm text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 p-2 rounded-lg">
             ...
             <span className="font-semibold">{contest.participantsCount} Joined</span>
           </div>
        </div>
        ...
        <div className="flex justify-between items-center text-sm text-gray-500 dark:text-gray-400 mb-6 border-t border-gray-100 dark:border-gray-800 pt-4">
          <div className="flex items-center" title="Deadline">
             <Calendar size={16} className={`mr-2 ${isEnded ? 'text-red-500' : 'text-orange-500'}`} />
             <span className="font-medium">{isEnded ? 'Ended' : new Date(contest.deadline).toLocaleDateString()}</span>
          </div>
        </div>
        ...
        >
          <span className="relative z-10 flex items-center">View Contest <ChevronRight size={16} className="ml-1 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all" /></span>
        </Link>
      </div>
    </motion.div>
  );
...
export const SectionTitle: React.FC<{ title: string; subtitle?: string }> = ({ title, subtitle }) => (
  <div className="text-center mb-16">
    <h2 className="text-3xl md:text-5xl font-black text-gray-900 dark:text-white mb-4 tracking-tight">{title}</h2>
    {subtitle && <p className="max-w-2xl mx-auto text-lg text-gray-500 dark:text-gray-400">{subtitle}</p>}
